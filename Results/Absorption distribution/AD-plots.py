...
for current_matrix in range(matrices):
    ...
    for folder in experiments:
        ...
        patches, text, autotext = subfig_list[current_matrix][row, 3].pie(pie_values[folder_number][current_matrix], labels=['$\mathcal{U}^{1}$', '$\mathcal{U}^{2}$', '$\mathcal{U}^{3}$'], labeldistance=1.2, autopct='$%.1f\%%$', wedgeprops=dict(width=0.08), colors=['#B3784B', '#78FF78', '#A591FF'], startangle=30, pctdistance=0.61-0.11, radius=1)
        ...
        if current_matrix == 3 and row == 1:
            subfig_list[current_matrix][row, 3].set_xlabel('$n_{i}$', fontsize=label_size, color='w')
            subfig_list[current_matrix][row, 3].set_xticks([0])
            subfig_list[current_matrix][row, 3].set_xticklabels([0], fontsize=tick_size, color='w')
